...
function handlePacket(data) {
	...
	if (data.cmd != "ping" and data.cmd != "pong")
		trace(data)
	...
	switch(cmd) {
		...
		case "lobby join":
			var lobby = data.lobby
			global.lobby = lobby
			global.status = "Waiting for opponent..."
		...
		case "color":
			if (!instance_exists(oChess))
				return queuePacket(data)
			
			oChess.local_color = data.color
			break
		case "board":
			//trace(data)
			...
			oChess.parseBoard(data.board)
			break
		case "possible moves":
			//trace(data)
			
			if(instance_exists(oChess)) {
				oChess.possible_from = data.from
				oChess.possible_moves = data.moves
			}
			else
				queuePacket(data)
			break
